...
| A saved plan replays the same instructions, and refuses to run against edited fixtures, a different planner version, or a plan that is malformed — anything `createPlan` could not have produced. Checked by fuzzing every single-field mutation of a plan. | It cannot reproduce a race that depended on machine timing. Use a barrier and a checkpoint for that. |
