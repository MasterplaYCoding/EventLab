...
| Reports never contain request bodies, and record request header *names* only. A response body is read up to the limit and then abandoned, so an endless one costs the limit rather than the request timeout. | Response body previews are captured (bounded, 64 KiB by default). If your target echoes secrets, redact them in your own handler. Abandoning the body means the connection is not reused; that is the intended trade. |
